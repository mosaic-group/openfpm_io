...
class CSVWriter
{
	...
	std::string get_csv_colums()
	{
		...
		boost::mpl::for_each_ref< boost::mpl::range_c<int,0,v_prp::value_type::max_prop> >(col);
	...
	std::string get_csv_data(v_pos & vp, v_prp & vpr, size_t offset)
	{
		...
		for (size_t i = offset ; i < vp.size() ; i++)
		{
			...
			boost::mpl::for_each_ref< boost::mpl::range_c<int,0,v_prp::value_type::max_prop> >(c_prp);
